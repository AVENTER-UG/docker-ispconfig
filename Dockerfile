...
echo -n "No eMail System Installed..."; fi

# Install Apache System
RUN IF ["isp_enable_apache" = "y"]; then \
# Install PhpMyAdmin
echo 'phpmyadmin phpmyadmin/dbconfig-install boolean true' | debconf-set-selections \
echo 'phpmyadmin phpmyadmin/mysql/admin-pass password pass' | debconf-set-selections \
apt-get -y install phpmyadmin \
./etc/phpmyadmin/phpmyadmin.ini /root/phpmyadmin.ini \
\
...
./etc/apache2/conf-available/httpoxy.conf /etc/apache2/conf-available/httpoxy.conf \
...
\
# PHP Opcode cache
apt-get -y install php7.4-opcache php-apcu \
\
# PHP 7.4 FPM
...
echo -n "No Apache2 System Installed..."; fi

# Install BIND DNS Server
RUN IF ["isp_enable_dns" = "y"]; then \
...
echo -n "No DNS System Installed..."; fi

# Install Jailkit
RUN IF ["isp_enable_jailkit" = "y"]; then \
apt-get -y install build-essential autoconf automake libtool flex bison debhelper binutils python \
cd /tmp \
...
echo -n "No Jailkit Installed..."; fi
